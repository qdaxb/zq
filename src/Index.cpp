...
struct IndexHandler : IndexSink {
    ...
    bool
    onLine(uint64_t lineNumber, const char *line, size_t length,
           size_t fileOffset) {
        try {
            indexed = false;
            ...
            indexer->index(*this, stringView, fileOffset);
            return indexed;
...
struct AlphaHandler : IndexHandler {
    std::map<std::string, std::map<uint32_t, std::list<uint64_t> *> *> &idxMap;
    std::hash<std::string> hasher;

    AlphaHandler(Log &log, std::unique_ptr<LineIndexer> indexer,
                 std::map<std::string, std::map<uint32_t, std::list<uint64_t> *> *> &map)
            : IndexHandler(log, std::move(indexer)),
              idxMap(map) {}

    void add(StringView key, size_t /*offset*/, std::string index,
             size_t fileOffset) override {
        indexed = true;
        log.debug("Found key '", key, "'");

        auto map = idxMap[index];
        auto hashed = ((uint32_t)hasher(key.str())) & 0xFFFFF;
        std::list<uint64_t> *list = nullptr;
        auto iter = map->find(hashed);
        if (iter == map->end()) {
            list = new std::list<uint64_t>();
            list->push_back(fileOffset);
            map->insert(
                    std::pair<uint32_t, std::list<uint64_t> *>(hashed,
                                                               list));
        } else {
            list = iter->second;
            list->push_back(fileOffset);
        }
//        std::cout << "offset:" << offset << ",line:" << currentLine
...
struct Index::Impl {
    ...
    Impl(Log &log, File &&fromCompressed, std::string indexFileName,
         Sqlite &&db)
            : log_(log), compressed_(std::move(fromCompressed)),
              indexFileName(std::move(indexFileName)),
              db_(std::move(db)),
              lineQuery_(db_.prepare(R"(
SELECT compressedOffset, uncompressedOffset,uncompressedEndOffset,  bitOffset, window FROM  AccessPoints
)")) {
        try {
...
//        }
        lineQuery_.reset();
        print(lineQuery_, lineSink, query, index);
        return 0;
    }
    ...
    struct checkpoint {
        ...
        size_t uncompressedEndOffset;
        size_t bitOffset;
        std::vector<uint8_t> window;
    };

    void
    print(Sqlite::Statement &q, LineSink &sink, const std::string &query,
          const std::string &index) {
        std::map<int, checkpoint> map;
        for (;;) {
            if (q.step()) break;
            checkpoint checkpoint;
            checkpoint.compressedOffset = static_cast<size_t>(q.columnInt64(0));
            checkpoint.uncompressedOffset = static_cast<size_t>(q.columnInt64(
                    1));
            checkpoint.uncompressedEndOffset = static_cast<size_t>(q.columnInt64(
                    2));
            checkpoint.bitOffset = static_cast<int>(q.columnInt64(3));
            checkpoint.window = q.columnBlob(4);
            map[checkpoint.compressedOffset] = checkpoint;
        }

        std::map<size_t, checkpoint> offsets;
        std::ifstream ifs(indexFileName + ".idx" + index,
                          std::ios::binary | std::ios::in);
        std::hash<std::string> hasher;
        auto hashed = ((uint32_t)hasher(query)) & 0xFFFFF;
        uint32_t key;
        long dropped[1024 * 32];
        while (true) {
            int size;
            uint32_t padding = 0;
            while (padding != 0xFFFFFFFEu) {
                if (!ifs.read((char *)&padding, 4)) {
                    goto loopOut;
                }
            }

            if (!ifs.read((char *)(&key), 4)) {
                goto loopOut;
            }
            ifs.read((char *)&size, 4);
            if (key != hashed) {
                while (size > 0) {
                    if (!ifs.read((char *)dropped,
                                  4 * std::min(size, 1024 * 32))) {
                        goto loopOut;
                    }
                    size -= 32 * 1024;
                }
            ...
            for (int j = 0; j < size; j++) {
                ...
                while (offset == 0xFFFFFFFF) {
                    if (!ifs.read((char *)&offset, 4)) {
                        goto loopOut;
                    }
                    shift++;
                }
                totalOffset = offset + (0xFFFFFFFEu * (long)shift);
                for (auto iter = map.begin(); iter != map.end(); iter++) {
                    if (totalOffset >=
                        (long)(iter->second.uncompressedOffset) &&
                        totalOffset <
                        (long)iter->second.uncompressedEndOffset) {
                        offsets[totalOffset] = iter->second;
                    }
                }
            }
        }
        ...
        for (auto iter = offsets.begin(); iter != offsets.end(); iter++) {
            checkpoint checkpoint = iter->second;
            auto offset = iter->first;
            auto uncompressedOffset = checkpoint.uncompressedOffset;
            auto bitOffset = checkpoint.bitOffset;
            auto compressedOffset = checkpoint.compressedOffset;
            ...
            if (cachedContext_ && cachedContext_->offsetWithinRange(offset)) {
                ...
                context.reset(
                        new CachedContext(uncompressedOffset, blockSize_));
                uint8_t window[WindowSize];
                uncompress(checkpoint.window, window, WindowSize);
            ...
            sink.onLine(0, offset,
                        query,
                        reinterpret_cast<const char *>(lineBuf.get()),
                        length - 1);
        }
    }
};
...
struct Index::Builder::Impl : LineSink {
    ...
    bool saveAllLines_;
    std::map<std::string, std::map<uint32_t, std::list<uint64_t> *> *> myMap;
    ...
    Impl(Log &log, File &&from, const std::string &fromPath,
         const std::string &indexFilename)
            : log(log), from(std::move(from)), fromPath(fromPath),
              ...
              addMetaSql(log), saveAllLines_{ false } {}
...
:compressedOffset, :bitOffset, :window))");
        ...
        do {
            if (zs.stream.avail_in == 0) {
                ...
                if (emitInitialAccessPoint && zs.stream.avail_in == 0 &&
                    feof(from.get())) {
                    break;
                }
                if (ferror(from.get()))
            ...
            do {
                ...
                if (endOfBlock && !lastBlockInStream && needsIndex) {
                    ...
                    last = totalOut;
                    std::cout << "indexing chunk..." << totalIn << std::endl;
        ...
        for (auto idxIter = myMap.begin(); idxIter != myMap.end(); idxIter++) {
            std::cout << "creating index " << idxIter->first << "..."
                      << std::endl;
            auto iter = idxIter->second->begin();
            std::ofstream fs(indexFilename + ".idx" + idxIter->first,
                             std::ios::binary | std::ios::out);


            uint32_t splitter = 0xFFFFFFFFu;
            uint32_t padding = 0xFFFFFFFEu;
            while (iter != idxIter->second->end()) {
                auto hashed = iter->first;
                int shift = 0;
                fs.write(reinterpret_cast<const char *>(&padding), 4);
                fs.write(reinterpret_cast<const char *>(&hashed), 4);
                auto list = iter->second;;
                int size = list->size();
                fs.write(reinterpret_cast<const char *>(&size), 4);
                for (auto listIter = list->begin();
                     listIter != list->end(); listIter++) {
                    long offset = *listIter;
                    if (shift > 0) {
                        offset = offset - (0xFFFFFFFEu * (long)shift);
                    }
                    while (offset >= 0xFFFFFFFEu) {
                        offset = offset - 0xFFFFFFFEu;
                        fs.write(reinterpret_cast<const char *>(&splitter), 4);
                        shift++;
                    }
                    fs.write(reinterpret_cast<const char *> (&offset), 4);
                }
                iter++;
            }
            fs.close();
    ...
    void addIndexer(std::vector<int> fields,
                    Index::IndexConfig /*config*/,
                    std::unique_ptr<LineIndexer> indexer) {

        for (auto iter = fields.begin(); iter != fields.end(); iter++) {
            myMap[std::to_string(
                    *(iter))] = new std::map<uint32_t, std::list<uint64_t> *>;
        }
        indexers.emplace("", std::unique_ptr<IndexHandler>(
                new AlphaHandler(log, std::move(indexer), myMap))
        );
    }
...
Index::Builder &Index::Builder::addIndexer(
        ...
        IndexConfig config,
        std::unique_ptr<LineIndexer> indexer) {
    impl_->addIndexer(fields, config, std::move(indexer));
    return *this;
}
...
size_t Index::queryIndex(const std::string &index, const std::string &query,
                         LineSink &lineSink) {
    return impl_->queryIndex(index, query, lineSink);
}
...
size_t Index::queryIndexMulti(const std::string &index,
    ...
    for (auto query: queries)
        result += impl_->queryIndex(index, query, lineSink);
    return result;
}
